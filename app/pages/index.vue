...
const { data: accountHistory, pending } = useFetch('/api/account-history?userId=1', {
    key: 'account-history-report',
    transform: (data: ListAccountsOverviewReturnSchema): ListAccountsOverviewReturnSchema => data.map(({title, totalBalance, change}) => ({ title, totalBalance, change}))
})
